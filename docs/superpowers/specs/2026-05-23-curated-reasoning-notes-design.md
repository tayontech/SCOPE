...
`config/cloudtrail-classes.json` remains useful, but its unknown-action fallback can overstate visibility. Most environments do not ingest CloudTrail data events, so unknown and data-event-only actions need uncertainty handling.
...
Keep `config/cloudtrail-classes.json`, but revise its model so unknown actions produce uncertain visibility rather than assumed management-event visibility.
