...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--run-dir' && argv[i + 1]) {
      ...
      console.log('       node scripts/enum/lambda.js --run-dir <dir> --regions <r1,r2,...>');
      console.log('');
      console.log('Options:');
...
async function main() {
  ...
  if (!args.runDir || (!args.region && !args.regions)) {
    ...
    console.error('       node scripts/enum/lambda.js --run-dir <dir> --regions <r1,r2,...>');
    process.exit(1);
  }
  ...
  try {
    if (regionList.length === 1) {
      ...
      const envelope = createEnvelope({ module: 'lambda', account_id: null, region: 'multi', status: 'complete', findings: allFindings });
