...
This subagent reads ONLY from `AUDIT_RUN_DIR/results.json` and `AUDIT_RUN_DIR/{service}.json` files.
...
For richer context on specific findings, you may read per-module JSON files from `AUDIT_RUN_DIR/`:
- `iam.json`, `ec2.json`, `s3.json`, etc.
