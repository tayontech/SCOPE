...
- Verify that roles referenced in replacements map to real role ARNs. If iam.json exists, check that role names in replacement filenames (`iam-replacement-{role-name}.json`) correspond to roles present in `$AUDIT_RUN_DIR/iam.json`.
- WARN if a replacement references a role not found in iam.json.
</review_policy_replacements>
