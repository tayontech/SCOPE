...
description: IAM policy replacement subagent — reads iam.json policy documents and staleness data, cross-references permission boundaries, produces full deployable replacement policy JSON per affected role. Dispatched by scope-defend orchestrator.
...
```bash
if [ ! -f "$AUDIT_RUN_DIR/iam.json" ]; then
  echo "STATUS: error"
  echo "ERRORS: iam.json not found in $AUDIT_RUN_DIR — IAM enumeration did not complete"
  exit 1
fi
...
**Primary data source: `AUDIT_RUN_DIR/iam.json`**

Read `AUDIT_RUN_DIR/iam.json` and extract per-role data:
...
From `iam.json`, extract the role's full policy state:
...
- A permission boundary attached to this role (in `permission_boundaries` from iam.json)
...
If any role's replacement fails (e.g., iam.json missing required fields), report it:
...
If iam.json is unreadable or results.json is missing entirely, report error and stop:
...
- If iam.json has no role data at all: STATUS error, stop
