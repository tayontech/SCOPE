...
After completing enumeration and attack path analysis, aggregate ALL module graph data into a single results.json. No inline HTML generation — the SCOPE dashboard at `http://localhost:3000` renders this data.
...
trust_relationships: [{ role_id, role_arn, principal, trust_type, is_wildcard, has_external_id, has_mfa_condition, risk }]
```
