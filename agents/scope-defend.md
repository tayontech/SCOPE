...
- `scp-deny-iam-privilege-escalation.json`
...
4. **Grouping logic:** If an attack path has 3+ detection_opportunity eventNames that represent the same behavior, group them into a single detection using OR. If they represent distinct attack phases (recon vs exploitation), create separate detections.
...
6. **One detection per distinct behavior.** If an attack path has 5 eventNames but 3 represent the same behavior (e.g., Attach*Policy variants), group them into one detection with OR. If they represent distinct behaviors (recon eventName vs exploitation eventName), create two separate detections.
...
All SPL detections consolidated for SOC import, organized by MITRE tactic in this order:
