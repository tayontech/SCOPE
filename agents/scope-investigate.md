...
and an interactive dashboard to ./investigate/investigate-YYYYMMDD-HHMMSS/dashboard.html
(Y/N):
```
...
| Dashboard | `$RUN_DIR/dashboard.html` | Interactive HTML dashboard — event timeline, query log |
...
3. Read `agents/scope-render.md` — generate HTML dashboard (PHASE=investigate, RUN_DIR=$RUN_DIR)
...
Generate the interactive HTML dashboard per `<dashboard_generation>` and write to `$RUN_DIR/dashboard.html`. Verify the file exists before referencing it.

**6. Confirm save:**

```
Saved to: ./investigate/investigate-YYYYMMDD-HHMMSS/
Dashboard: ./investigate/investigate-YYYYMMDD-HHMMSS/dashboard.html — open in browser to explore
```
...
scope-render reads normalized data from `./data/investigate/<run-id>.json`
...
6. If the analyst chose to save: `dashboard.html` written to `$RUN_DIR/`, verified with `test -f`, and link printed alongside `investigation.md` path
