...
import scope_runtime.__main__ as cli
import scope_runtime.audit as audit
...
def test_run_work_item_invokes_scope_runtime_with_current_python(monkeypatch, tmp_path: Path):
    item = audit.WorkItem(
        module="sns",
        region="us-east-1",
        enum_run_dir=tmp_path / ".module-runs" / "sns" / "us-east-1",
    ...
    assert commands[0][:3] == [sys.executable, "-m", "scope_runtime"]
