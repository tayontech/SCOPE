...
import pytest
import scope.runtime.__main__ as scope_main
import scope.core.base_enum as base_enum
from scope.core.base_enum import dispatch, main_for
...
def test_scope_cli_rejects_unknown_module_before_dispatch(monkeypatch, tmp_path: Path):
    def fail_dispatch(**kwargs):
        raise AssertionError("dispatch should not run for invalid module names")

    monkeypatch.setattr(scope_main, "dispatch", fail_dispatch)

    with pytest.raises(SystemExit):
        scope_main.main(["enum", "typo", "--run-dir", str(tmp_path)])


def test_scope_cli_enum_wires_imported_module_to_dispatch(monkeypatch, tmp_path: Path, capsys):
    calls = {}
    ...
    monkeypatch.setattr(scope_main, "ClientFactory", FakeClientFactory)
    monkeypatch.setattr(scope_main.importlib, "import_module", fake_import_module)
    monkeypatch.setattr(scope_main, "dispatch", fake_dispatch)

    result = scope_main.main(["enum", "sts", "--run-dir", str(tmp_path), "--region", "global", "--profile", "dev"])

    assert result == 0
    assert calls["module_name"] == "sts"
    assert calls["import_name"] == "scope.enumerators.sts"
...
def test_scope_cli_enum_parses_selector_json_for_dispatch(monkeypatch, tmp_path: Path):
    calls = {}
...
def test_scope_cli_invalid_selector_json_writes_error_envelope(tmp_path: Path):
    result = scope_main.main(["enum", "sts", "--run-dir", str(tmp_path), "--selector-json", "{"])
...
def test_scope_cli_enum_maps_lambda_service_to_import_safe_module(monkeypatch, tmp_path: Path):
    calls = {}
    ...
    result = scope_main.main(["enum", "lambda", "--run-dir", str(tmp_path), "--region", "us-east-1"])

    assert result == 0
    assert calls["module_name"] == "lambda"
    assert calls["import_name"] == "scope.enumerators.aws_lambda"


def test_scope_cli_missing_valid_module_writes_error_envelope(monkeypatch, tmp_path: Path, capsys):
    class FakeClientFactory:
        account_id = "123456789012"
    ...
    def fake_import_module(name: str):
        assert name == "scope.enumerators.s3"
        raise ModuleNotFoundError("No module named 'scope.enumerators.s3'")

    monkeypatch.setattr(scope_main, "ClientFactory", FakeClientFactory)
    monkeypatch.setattr(scope_main.importlib, "import_module", fake_import_module)

    result = scope_main.main(["enum", "s3", "--run-dir", str(tmp_path), "--region", "us-east-1"])
...
def test_scope_cli_factory_construction_failure_writes_error_envelope(monkeypatch, tmp_path: Path):
    ...
    def fake_import_module(name: str):
        assert name == "scope.enumerators.sts"
        return SimpleNamespace(run=imported_run)

    monkeypatch.setattr(scope_main, "ClientFactory", FailingClientFactory)
    monkeypatch.setattr(scope_main.importlib, "import_module", fake_import_module)

    result = scope_main.main(["enum", "sts", "--run-dir", str(tmp_path), "--profile", "missing"])
    payload = json.loads((tmp_path / "sts.json").read_text(encoding="utf-8"))
...
def test_scope_cli_valid_module_missing_run_writes_error_envelope(monkeypatch, tmp_path: Path):
    class FakeClientFactory:
        account_id = "123456789012"
