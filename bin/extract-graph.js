...
function main() {
  ...
  // Read module files (missing = empty resources)
  ...
  // Extract nodes
  const nodes = withEdgeEndpointNodes(
    extractNodes(iam, s3, kms, secrets, rds, lambda, ec2, codebuild, apigateway, sns, sqs, bedrock, cognito, dynamodb, ssm),
    edges
  );
 ...
 * Current audit runs write modules/<service>/<region>.json. Top-level
 * <service>.json is still supported for archived runs and older fixtures.
 * Returns parsed object. If file doesn't exist, returns {resources: []}.
...
function readModule(runDir, filename) {
  ...
  const service = path.basename(filename, '.json');
  const moduleDir = path.join(runDir, 'modules', service);
  if (!fs.existsSync(moduleDir) || !fs.statSync(moduleDir).isDirectory()) {
    return { resources: [] };
  }

  const resources = [];
  const regionFiles = fs.readdirSync(moduleDir)
    .filter(name => name.endsWith('.json'))
    .sort();
  for (const regionFile of regionFiles) {
    const raw = fs.readFileSync(path.join(moduleDir, regionFile), 'utf-8');
    resources.push(...resourcesOf(JSON.parse(raw)));
  }
  return { resources };
}

/**
 * Current Python enumerators emit top-level resources. Legacy JS fixtures and
 * archived run directories may still contain findings.
 */
 ...
 * - OIDC providers: oidc: (from iam.json oidc_provider resources)
...
function extractNodes(iam, s3, kms, secrets, rds, lambda, ec2, codebuild, apigateway, sns, sqs, bedrock, cognito, dynamodb, ssm) {
  ...
  for (const finding of resourcesOf(iam)) {
  ...
  for (const finding of resourcesOf(ssm)) {
  ...
  // Compute tier
  for (const finding of resourcesOf(lambda)) {
  ...
  for (const finding of resourcesOf(ec2)) {
  ...
  for (const finding of resourcesOf(codebuild)) {
  ...
  // Gateway tier
  for (const finding of resourcesOf(apigateway)) {
  ...
  // Messaging tier
  for (const finding of resourcesOf(sns)) {
  ...
  for (const finding of resourcesOf(sqs)) {
  ...
  // AI tier
  for (const finding of resourcesOf(bedrock)) {
  ...
  for (const finding of resourcesOf(cognito)) {
...
function extractEdges(iam, lambda, ec2, codebuild, bedrock, apigateway, cognito) {
  ...
  // OIDC provider -> IAM: authenticates_to roles
  for (const finding of resourcesOf(iam)) {
    if (finding.resource_type === 'oidc_provider') {
...
function withEdgeEndpointNodes(nodes, edges) {
  const existing = new Set(nodes.map(n => n.id));
  const inferred = [];

  for (const edge of edges) {
    for (const endpoint of [edge.source, edge.target]) {
      if (!endpoint || existing.has(endpoint)) continue;
      inferred.push(nodeFromEndpoint(endpoint));
      existing.add(endpoint);
    }
  }

  return deduplicateAndSort([...nodes, ...inferred], n => n.id);
}

function nodeFromEndpoint(id) {
  return {
    id,
    label: labelFromEndpoint(id),
    type: typeFromEndpoint(id),
    _source: 'api'
  };
}

function labelFromEndpoint(id) {
  const prefixes = [
    'compute:lambda:',
    'compute:ec2:',
    'compute:codebuild:',
...
function typeFromEndpoint(id) {
  ...
  if (id.startsWith('idp:')) return 'idp';
  if (id.startsWith('oidc:')) return 'oidc';
  return 'external';
}
