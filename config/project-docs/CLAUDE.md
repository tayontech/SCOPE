...
**Installation:** Run `node bin/install.js` to copy hook scripts to `.claude/hooks/` and hook settings to `.claude/settings.json`. The installer rewrites hook paths to absolute references so hooks resolve correctly regardless of CWD.
...
- **Intel mode** (invoked with a threat intel URL or natural language threat description): fetches the URL or parses the description, extracts IOCs and TTPs, generates hypotheses beyond the report, and hunts in Splunk. Extracted identifiers are session-scoped.
