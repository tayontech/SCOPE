...
`scope-data` normalizes raw agent artifacts into structured JSON in `./data/`. `scope-evidence` validates and indexes `evidence.jsonl` into provenance envelopes in `./evidence/`. `scope-render` generates HTML dashboards from normalized data. All three run automatically as a sequential pipeline after each agent writes artifacts — no operator interaction.
