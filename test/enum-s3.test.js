...
const { run } = require('../scripts/enum/s3');

const FIXTURES = path.join(__dirname, 'fixtures', 'enum', 's3');
...
async function runTests() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scope-s3-test-'));
  ...
  try {
    ...
    try {
      ...
      console.error('  FAIL: s3 enum output mismatch');
