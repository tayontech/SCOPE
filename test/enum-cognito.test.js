...
const { run } = require('../scripts/enum/cognito');
const FIXTURES = path.join(__dirname, 'fixtures', 'enum', 'cognito');
...
async function runTests() {
  ...
  // --- Test: basic scenario ---
  try {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scope-test-cognito-'));

    // CognitoIdentityClient mock
    const mockCognitoIdentity = makeMockClient({
      ListIdentityPoolsCommand: apiResponses['ListIdentityPoolsCommand'],
      DescribeIdentityPoolCommand: apiResponses['DescribeIdentityPoolCommand'],
    });

    // CognitoIdentityProviderClient mock
    const mockCognitoIdp = makeMockClient({
      ListUserPoolsCommand: apiResponses['ListUserPoolsCommand'],
      DescribeUserPoolCommand: apiResponses['DescribeUserPoolCommand'],
      ListUserPoolClientsCommand: apiResponses['ListUserPoolClientsCommand'],
    ...
    console.error('  FAIL: cognito basic scenario');
