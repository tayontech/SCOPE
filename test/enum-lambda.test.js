...
const { run } = require('../scripts/enum/lambda');

const FIXTURES = path.join(__dirname, 'fixtures', 'enum', 'lambda');
...
async function runTests() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scope-lambda-test-'));
  ...
  try {
    ...
    try {
      ...
      console.error('  FAIL: lambda enum output mismatch');
