...
const { run } = require('../scripts/enum/secrets');

const FIXTURES = path.join(__dirname, 'fixtures', 'enum', 'secrets');
...
async function runTests() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scope-secrets-test-'));
  ...
  try {
    ...
    try {
      ...
      console.error('  FAIL: secrets enum output mismatch');
