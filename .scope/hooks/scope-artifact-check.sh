...
if [ -n "$LATEST_AUDIT" ]; then
  ...
  if [ ! -f "$LATEST_AUDIT/evidence.jsonl" ]; then
    ERRORS+=("MISSING: $LATEST_AUDIT/evidence.jsonl (mandatory audit artifact)")
  fi
...
if [ -n "$LATEST_EXPLOIT" ]; then
  ...
  if [ ! -f "$LATEST_EXPLOIT/evidence.jsonl" ]; then
    ERRORS+=("MISSING: $LATEST_EXPLOIT/evidence.jsonl (mandatory exploit artifact)")
  fi
