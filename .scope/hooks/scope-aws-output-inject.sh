...
# Defensive check — matcher already filters, but be consistent with safety guard pattern.
...
# Use word-boundary regex to match "aws " at start or after whitespace,
# preventing false matches on paths like /usr/local/awscli-local or env vars.
